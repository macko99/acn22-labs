...
#region Fattree
    ...
    #create list of all possible pairs of hosts (combination)
    #in fact we just consider next hop from each host (there is just one)
    #this allows us to run dijkstra for less nodes (just switches) 
    #than we will just add distance/cost of 2 (or some bandwidth) to the distance between two hosts connecting by those next-hops
