...
#function creating jellyfish topology following the paper
	def generate(self, num_servers, num_switches, num_ports):
		...
		for i in range(num_servers):
			...
			#Server was connected to swithch so we decrease number of open ports on switch
		...
		#We keep track how many more swiches need to be connected
		...
		while switch_left > 1 and switch_idx_pairs:
			...
			if open_ports[pair[0]] > 0 and open_ports[pair[1]] > 0:
				...
				#Deacrising number of open ports on both switches
				open_ports[pair[0]] -= 1
				open_ports[pair[1]] -= 1
		...
		#after connecting random pairs of swiches there still might be switches not utilized (having more than 1 open port)
		while self.not_utilized_switches_exists(open_ports, num_switches):
			for idx, switch3 in enumerate(switches):
				if open_ports[idx] > 1:
					...
					pair_idx_2 = idx
					old_edge = None
					#we look for pair of switches that are is connected but is not our neighbour (like in the paper)
...
class Fattree:
	...
	def generate(self, num_ports):
		...
		#in each pod we seperate adding lower layer switches with hosts and upper layer switches
	...
	def plot(self, num_ports, save):
		#sclaing the plot based on num_ports
		scale = 5 if num_ports > 6 else 3
...
def dijkstra(start_vertex, verticies):
	...
	#we init the priority queue, used to put possible paths from current node
    p_queue = queue.PriorityQueue()
    p_queue.put(Prioritize(0, start_vertex))

	#we run the algorith untill we vae possible move (node in the queue)
...
#utility fucntion for priting shortest path calculated in dijkstra algorithm
def print_path(previous, start_node, end_node):
	...
	#we traveresd the path backwards, so reverse the list
    path.reverse()
...
#utility fucntion for getting the path between two nodes, based on previous list from dijkstra algorithm
...
#utility fucntion for getting the path between two nodes, it executes the dijkstra algorithm
def dijkstra_get_path(start_vertex, end_node, verticies):
    distance, previous = dijkstra(start_vertex, verticies)

	#it is possible that path between two nodes does not exists!
...
#region Yen
def ksp_yen(verticies, node_start, node_end, max_k):
    distances, previous = dijkstra(node_start, verticies)
    ...
    for _ in range(1, max_k):
        ...
        for i in range(0, len(A[-1]['path']) - 1):
            ...
            _, path_spur = dijkstra_get_path(node_spur, node_end, verticies)
